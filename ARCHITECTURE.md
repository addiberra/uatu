...
| `background_task` | A task the agent runs in the background, updated in place from start to settling. Running ones are listed above the composer with a stop control and open for inspection; settled ones are rows. | Composer list, drill-down, timeline |
...
A skill Claude Code runs as a fork of itself is the opposite case, and the reason the provider buffers. A fork's frames stream under the Skill tool use from the first moment, but its `agentId` appears only in the tool result, when the run ends — so until then there is no id to open, and minting a provisional one would mean renaming the conversation a drill-down had subscribed to. A tagged frame whose run is not yet known is therefore held against its tool use (`forkBuffers`, bounded both in waiting tool uses and in frames per tool use, with stream deltas never held — the complete frame that follows says the same thing) and still falls through to the parent's normalizer, which is what keeps the fork's tool activity visible in the timeline while it runs. When a result carries `status: "forked"` with an `agentId`, the child opens running, the buffer replays into it in arrival order through the same normalizer and the same child memory as a live frame would use, and the settle comes last — so the transcript is *complete* the first moment it can be read, rather than starting at the end. Meanwhile the subagents track names the fork from its launching row (`Skill · <name>`, carrying the latest tool activity in the parent timeline as its progress note, attributed positionally because nothing in a tool row says which run produced it) and offers it as an open button only once the result has named it.
