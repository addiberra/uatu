...
- **Published documentation:** the worktree API (`preflight-delete` and `delete`), the OpenAPI contract, the unreleased Hub 8 changelog entry, `docs/WORKTREES.md` and `ARCHITECTURE.md` describe the acknowledgement and the single-force removal. This change deliberately reverses the archived "no force override in this scope" decision for acknowledged local data only.
