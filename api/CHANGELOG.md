...
- The live stream's `WorkspaceActivity` gains a required `finished` boolean, held per caller: the Hub observed the workspace working, it has since gone quiet, and the caller has not viewed its chat since. `working` now also covers an agent's live background work after its turn ended. A new Hub operation, `POST /s/{workspaceId}/api/activity-viewed` (`hubAcknowledgeWorkspaceActivity`, 204), reports that the caller has the workspace's chat in view and clears `finished` for that caller on every device.
...
Strict Hub clients must regenerate against Hub revision 8. The state objects are closed, so revision 7 validators reject the new optional fields, and the closed `WorkspaceActivity` object rejects `finished`. Accept the new `worktrees` envelope topic and subscription variant. A client that shows the activity summary should send `hubAcknowledgeWorkspaceActivity` while it has a workspace's chat in view, or `finished` stays set for that user until the session stops or work starts again. On invalidation, fetch authoritative inventory without changing the selected workspace or conversation. The workspace payload revision remains 20.
