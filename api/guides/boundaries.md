...
Personal state is the exception under that prefix. The Hub stores it per user and workspace and answers `/s/{workspaceId}/api/personal-state` itself, whether or not the workspace is running, so `workspaceGetPersonalState` and `workspacePatchPersonalState` are public Hub operations. The activity acknowledgement, `POST /s/{workspaceId}/api/activity-viewed` (`hubAcknowledgeWorkspaceActivity`), is served the same way. What the Hub serves is public, and what it proxies to the child is internal.

Workspace updates reach public clients through the Hub's live stream, `GET /api/hub/live`. A client subscribes to one workspace's document state, its conversation inventory, and the conversations it follows. It can also ask for an activity summary of every workspace it may access: whether each one is running, whether an agent is working, whether anything awaits the user, and whether work there finished since the caller last viewed its chat. The document, inventory, and conversation payloads come from the workspace, so `workspaceApiRevision` versions them.
